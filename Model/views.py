...
def Results(request, financial_year):
    ...
    chart_data = {
        'labels': ['Anthracite', 'Bituminous Coking', 'Bituminous Non-Coking', 'Subbituminous', 'Lignite'],
        ...
        'explosives_used': emission.explosives_used,
        'transport_distance': emission.transport_distance
    }
    
    # Prepare data for the pie chart (processes)
    pie_chart_data = {
        'values': [
            emission.anthracite,
            emission.bituminous_coking,
            emission.bituminous_non_coking,
            emission.subbituminous,
            emission.lignite,
            emission.explosives_used,
            emission.transport_distance,
            emission.diesel_used,
            emission.petrol_used,
            emission.electricity_used,
            emission.overburden_removed,
            emission.land_disturbance,
            emission.total_ch4,
            emission.waste
        ],
        'labels': [
    ...
    # Convert chart data to JSON format for use in the template
    chart_data_json = mark_safe(json.dumps(chart_data))
    pie_chart_data_json = mark_safe(json.dumps(pie_chart_data))
    
    return render(request, 'Calculator/Results.html', {'emission': emission, 'chart_data': chart_data_json, 'pie_chart_data': pie_chart_data_json})      
